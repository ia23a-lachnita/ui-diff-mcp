...
export function buildAuditorPrompt(ctx: AuditorPromptContext): string {
  ...
  return [
    ...
    `- Transparent bands or pixels in the normalized actual comparison crop are synthetic contain padding (pillarbox/letterbox) added to align the actual image to the expected dimensions; they are not missing UI and must not be reported as a difference.`,
