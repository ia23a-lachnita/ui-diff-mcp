...
- For production plans, debugging help, and implementation reviews, request `approvalMode: "yolo"` and a persistent `conversationId` so revisions can be reviewed in the same conversation without interactive stalls. Route models in this canonical fallback order: (1) `gemini-3.8-flash` primary, (2) `gemini-3.7-flash` fallback, (3) `gemini-3.6-flash` final fallback. High effort is supplied separately; display labels are not passed to the API. `gemini-3.1-pro` is opt-in only for explicitly justified maximum reasoning; on failure, return to `gemini-3.8-flash`. `gemini-3.5-flash` (SDK/OpenRouter-only) is removed from agy external review only, not internal UI-diff VLM routes. Record the exact error (category, provider message, timestamp) for each failed route before trying the next.
